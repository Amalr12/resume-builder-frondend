...
function Edit1({ EditID,setResumeData }) {
    ...
    const addSkill = (adskill) => {
        ...
        if (!adskill) {
            ...
            if (editResumeData.skills.includes(adskill)) {
                toast.warning("Skill already added..");
            } else {
                setEditResumeData({ ...editResumeData, skills: [...editResumeData.skills, adskill] });
            }
            setEditInputSkill("");
        }
    };
    ...
    const handleUpdate = async () => {
        try {
            const result = await updateAResumeAPI(EditID, editResumeData)
            console.log(result);
