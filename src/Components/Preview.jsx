...
function Preview({ resumeData, EditID, setResumeData, isFinished }) {
  ...
  const downloadPDF = async () => {
    ...
    const canvas = await html2canvas(input, { scale: 2 })//convert the selected 
      ...
      <div id='result' className="mx-5 my-3 shadow px-5 pb-5">
        ...
        <div>
          ...
          <div className="text-center">
            ...
            <h2 className="">{resumeData?.name || "your name"}</h2>
          ...
          <div className='mt-2'>
            {/* {
              resumeData?.skills.map((item) => (
                <button className='me-2 mt-2 btn btn-primary'>{item}</button>
              ))
            } */}
            {
              resumeData?.skills.map((item) => (
